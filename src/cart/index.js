...
import OrderSuccessModal from  "../components/Layout/UI/OrderSuccess"
import { clearCartHandler,addItemHandler,removeItemHandler } from "../actions"
                        ...
                        <div className="checkout-modal_list">
                            {
                                items.length > 0 ? items.map((item)=>{
                                    return <CartItem data={item} key={item.id} onEmitIncreaseItem={item=>dispatchEvents(1,item)} onEmitDecreaseItem={item=>dispatchEvents(-1,item)}/>
